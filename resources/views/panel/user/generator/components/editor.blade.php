<div class="lqd-generator-v2-editor">
    ...
    <div class="container relative">
        <div class="mx-auto lg:w-2/3">
            ...
            >
                <textarea
                    class="tinymce w-full border-none"
                    id="default"
                    rows="25"
                ></textarea>
            </form>
...
@push('script')
    ...
    <script src="/assets/js/panel/openai_generator_workbook.js?v={{ time() }}"></script>
        ...
        $(document).on('click', '#openai_generator_button', function() {
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    Alpine.$data(document.querySelector('.lqd-generator-v2')).toggleSideNavCollapse('collapse');
                    ...
                    @if ($setting->hosting_type == 'high')
                        ...
                        var link = '/dashboard/user/openai/generate?message_id=';

                        link = link + message_id + "&maximum_length=" + data.maximum_length + "&number_of_results=" + data.number_of_results + "&creativity=" + data
                            .creativity;

                        if (openai_type === "youtube") {
                            link = link + "&youtube_url=" + $("#url").val();
                        }

                        if (openai_type === 'rss') {
                            link = link + "&rss_image=" + $("#title :selected").attr('data-image');
                        }
