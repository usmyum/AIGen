<div class="row text-[13px] items-center">
    <div class="col-12 ms-auto flex items-center">
            ...
            <div class="flex rtl:flex-row-reverse">
                <button
                    ...
                    <svg width="16" height="17" viewBox="0 0 16 17" fill="none"
                        ...
                        <path d="M4.28674 7.7378L2.58008 6.03113L4.28674 4.32446" stroke="var(--lqd-heading-color)"
                ...
                <button
                    ...
                    <svg width="16" height="17" viewBox="0 0 16 17" fill="none"
                        ...
                        <path
                            ...
                            stroke="var(--lqd-heading-color)" stroke-width="1.25" stroke-miterlimit="10"
                            stroke-linecap="round" stroke-linejoin="round" />
                        <path d="M11.7134 7.7378L13.42 6.03113L11.7134 4.32446" stroke="var(--lqd-heading-color)"
                            stroke-width="1.25" stroke-linecap="round" stroke-linejoin="round" />
                    </svg>
        ...
        @if ($workbook->generator->type != 'code')
            <div class="relative">
                <a class="bg-transparent p-1 inline-flex items-center justify-center rounded-sm w-[30px] h-[30px] border-0 text-[13px] hover:!bg-[var(--lqd-faded-out)] transition-colors"
                    title="{{ __('Download') }}"
                    @if ($workbook->generator->type == 'voiceover') href="/uploads/{{ $workbook->output }}"
						target="_blank" 
						@else
                ...
                <div
                    ...
                    <div class="flex flex-col p-1 gap-1">
                        <button
                            ...
                            data-doc-type="doc" data-doc-name="{{ $workbook->title }}">
                        ...
                        <button
                            class="workbook_download flex items-center gap-1 p-2 border-none rounded-md bg-[transparent] text-[12px] font-medium text-heading hover:bg-slate-100 dark:hover:bg-zinc-900"
                            data-doc-type="html" data-doc-name="{{ $workbook->title }}">
                            <svg class="shrink-0" xmlns="http://www.w3.org/2000/svg" width="24" height="24"
                                viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" fill="none"
                                stroke-linecap="round" stroke-linejoin="round">
        ...
        @endif
        <a href="{{ LaravelLocalization::localizeUrl(route('dashboard.user.openai.documents.delete', $workbook->slug)) }}"
...
@if ($workbook->generator->type == 'code')
    <input id="code_lang" type="hidden"
        value="{{ substr($workbook->input, strrpos($workbook->input, 'in') + 3) }}">
    <div
        class="min-h-full border-solid border-t border-r-0 border-b-0 border-l-0 border-[var(--tblr-border-color)] pt-[30px] mt-[15px] max-lg:mt-0 max-lg:pt-0 max-lg:border-t-0">
...
@elseif(in_array($workbook->generator->type, ['text', 'youtube', 'rss']))
    <div
        class="border-solid border-t border-r-0 border-b-0 border-l-0 border-[var(--tblr-border-color)] pt-[30px] mt-[15px] [&_.tox-edit-area__iframe]:!bg-transparent">
        <form class="workbook-form" onsubmit="editWorkbook('{{ $workbook->slug }}'); return false;" method="POST">
            ...
            <div class="mb-[20px]">
                <input type="text" class="form-control rounded-md" placeholder="{{ __('Untitled Document...') }}"
                    id="workbook_title" value="{{ $workbook->title }}">
            </div>
            <div class="mb-[20px]">
