        ...
        <div class="container-xl">
            <div class="card">
                <div class="card-header d-flex justify-content-between">
                    <h4>{{ __('Voice training') }}</h4>
                    <a class="btn btn-primary" href="{{ route('dashboard.user.voice.create') }}">{{ __('Add New') }}</a>
                </div>
                    ...
                    <table class="table">
                        <thead>
                            <tr>
                                ...
                                <th><button class="table-sort" data-sort="sort-name">{{ __('Voice id') }}</button></th>
                        ...
                        <tbody class="table-tbody align-middle text-heading">
                            ...
                            @foreach ($items as $item)
                                <tr>
                                    ...
                                    <td class="sort-name">{{ $item->voice_id }}</td>
                                    <td class="sort-group"> <span
                                            class="badge bg-{{ $item->status ? 'success' : 'danger' }}">{{ $item->status ? __('Active') : __('Inactive') }}</span>
                                    </td>
                                    <td class="sort-date" data-date="{{ strtotime($item->created_at) }}">
                                        <p class="m-0 text-muted">{{ date('H:i:s', strtotime($item->created_at)) }}</p>
                                    </td>
                                    <td class="!text-end whitespace-nowrap">
                                        <a href="{{ route('dashboard.user.voice.edit', $item->id) }}"
                                        ...
                                        <a href="{{ route('dashboard.user.voice.destroy', $item->id) }}"
