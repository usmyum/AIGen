        ...
        <div class="container-xl">
            <div class="row g-2 items-center">
                <div class="col-md-6">
                    ...
                    <p class="mb-2">Train MagicAI on your own data (website or PDF) and make your AI content exclusive.
                    </p>
                </div>
                <div class="col-md-6 d-flex justify-end align-center items-center align-items-center">
                    <a class="btn mr-2 " href="{{ route('dashboard.admin.chatbot.setting') }}">@lang('Settings')</a>
                    <a class="btn btn-primary" href="{{ route('dashboard.admin.chatbot.create') }}">
                ...
                <div id="table-default" class="card-table table-responsive">
                            ...
                            @foreach ($items as $item)
                                <tr>
                                    <td class="sort-name">
                                        @if ($item->image)
                                            <img src="{{ $item->image_url }}"
                                                class="mr-2 img-fluid rounded w-[50px] h-[50px]" alt="{{ $item->title }}">
                                        @else
                                            <div class="inline-flex items-center justify-center  w-[60px] h-[60px] rounded mx-auto  transition-shadow text-[20px] font-medium overflow-hidden border-solid border-[6px] !border-white  whitespace-nowrap overflow-ellipsis  mr-2 "
                                                style="background: #EACCEB;">
                                                <span
                                                    class="block w-full text-center whitespace-nowrap overflow-hidden overflow-ellipsis">{{ \Illuminate\Support\Str::limit($item->title, 2, '') }}</span>
                                            </div>
                                        @endif
                                        {{ $item->title }}
                                    </td>
                                    ...
                                    <td class="text-center">
                                        <a href="{{ route('dashboard.admin.chatbot.show', $item->id) }}"
                                            class="btn w-[36px] h-[36px] p-0 border hover:bg-[var(--tblr-primary)] hover:text-white open-modal-link mr-2"
                                            ...
                                            <svg width="17" height="16" viewBox="0 0 17 16" fill="none"
                                                ...
                                                <defs>
                                                    <linearGradient id="paint0_linear_3314_1636" x1="1.03221e-07"
                                                        y1="3.30635" x2="13.3702" y2="15.6959"
                                        ...
                                        <a href="{{ route('dashboard.admin.chatbot.edit', $item->id) }}"
                                            class="btn w-[36px] h-[36px] p-0 border hover:bg-[var(--tblr-primary)] hover:text-white mr-2"
                                            title="{{ __('Edit') }}">
                    ...
                    @if ($items->total() > 10)
                        <div
                            ...
                            <div class="m-0 ms-auto p-0">{{ $items->links() }}</div>
                        </div>
                    @endif
                </div>
            </div>
