            ...
            @if ($gatewayError == true)
                <div class="row">
                    <div class="col-md-12 mb-3">
                        <div
                            class="bg-amber-100 text-amber-600 rounded-xl !p-3 !mt-2 dark:bg-amber-600/20 dark:text-amber-200">
                            ...
                            <ul>
                                ...
                                <li>{{ __('Beware of that order is important. First set mode then save gateway settings.') }}
                                </li>
                            </ul>
                        </div>
                    </div>
                                ...
                                <div class="col-auto">
                                    <span class="avatar bg-white dark:!bg-[rgba(255,255,255,0.05)]">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="23" height="23"
                                            viewBox="0 0 24 24" stroke-width="1.5" stroke="var(--lqd-heading-color)"
                ...
                <div class="col-lg-6">
                    <div class="card">
                        ...
                        <div class="card-body">
                            ...
                            <div class="hidden text-[var(--tblr-green)] bg-[rgba(var(--tblr-green-rgb),0.15)] -scale-100">
                            </div>
                            ...
                            <h3 class="flex items-center">
                                ...
                                @endif
                                <span
                                    class="inline-flex items-center leading-none !ms-2 text-[var(--tblr-green)] text-[10px] bg-[rgba(var(--tblr-green-rgb),0.15)] px-[5px] py-[3px] rounded-[3px]">
                                    <svg class="mr-1" width="7" height="4" viewBox="0 0 7 4"
                                        fill="currentColor" xmlns="http://www.w3.org/2000/svg">
                                        <path
                                            d="M0 3.2768C0 3.32591 0.0245541 3.38116 0.061384 3.41799L0.368304 3.72491C0.405134 3.76174 0.46038 3.78629 0.509487 3.78629C0.558594 3.78629 0.61384 3.76174 0.65067 3.72491L3.06306 1.31252L5.47545 3.72491C5.51228 3.76174 5.56752 3.78629 5.61663 3.78629C5.67188 3.78629 5.72098 3.76174 5.75781 3.72491L6.06473 3.41799C6.10156 3.38116 6.12612 3.32591 6.12612 3.2768C6.12612 3.2277 6.10156 3.17245 6.06473 3.13562L3.20424 0.275129C3.16741 0.238299 3.11217 0.213745 3.06306 0.213745C3.01395 0.213745 2.95871 0.238299 2.92188 0.275129L0.061384 3.13562C0.0245541 3.17245 0 3.2277 0 3.2768Z" />
                                    </svg>
                ...
                <div class="col-lg-6">
                    <div class="card">
                        ...
                        <div class="card-body w-full !grow-0 mt-auto">
                            <div id="chart-daily-usages"
                                class="[&_.apexcharts-legend-text]:ps-2 [&_.apexcharts-legend-text]:!pe-2 [&_.apexcharts-legend-text]:!m-0">
                            </div>
                            <div class="chart-navigation flex items-center gap-1 absolute top-2 end-2">
                                <button type="button" id="btnPreviousMonth"
                                    class="inline-flex w-7 h-7 items-center justify-center border-none bg-none  dark:bg-white/10 dark:text-white">
                                ...
                                @foreach ($activity as $entry)
                                    <tr>
                                        ...
                                        <td class="!text-end">
                                            @if (isset($entry->url))
                                                <a href="{{ $entry->url }}"
                                                    class="btn btn-sm btn-primary">{{ __('Go') }}</a>
                                            @endif
                                        </td>
                                    </tr>
                                @endforeach
                            ...
                            @if (count($activity) == 0)
                                <div class="h-full flex flex-col items-center justify-center text-center overflow-hidden">
                                    <svg xmlns="http://www.w3.org/2000/svg"
                                        class="icon icon-tabler icon-tabler-article-off icon-lg mb-2 opacity-40"
                            ...
                            <table class="table table-vcenter">
                                ...
                                @foreach ($latestOrders as $order)
                                    <tr>
                                        ...
                                        <td class="text-muted">
                                            <span
                                                class="text-[var(--lqd-heading-color)]">{{ $order->user->fullName() }}</span>
                                            <br>
                                            <span class="opacity-70">{{ __($order->type) }}</span>
                                        </td>
                                        <td class="w-1" colspan="3">
                                            <span
                                                class="text-primary font-medium">{{ @$order->plan->name ?? 'Archived Plan' }}</span>
                                            /<span
                                                class="text-[var(--lqd-heading-color)] !ms-1">{{ @$order->plan->total_words === '-1' ? __('Unlimited') : @$order->plan->total_words ?? '-' }}</span>
                                            /<span
                                                class="text-[var(--lqd-heading-color)] !ms-1">{{ @$order->plan->total_images === '-1' ? __('Unlimited') : @$order->plan->total_images ?? '-' }}</span>
                                        </td>
                                    </tr>
                                @endforeach
        ...
        (() => {
            ...
            var dailySalesChartOptions = {
                series: [{
                    name: 'Sales',
                    data: [
                        @foreach (json_decode(cache('daily_sales')) ?? [] as $dailySales)
                            [{{ strtotime($dailySales->days) * 1000 }},
                                {{ $dailySales->sums }}
                            ],
                        @endforeach
                    ]
            ...
            var dailyUsageChartOptions = {
                series: [{
                    name: 'Words',
                    data: [
                        @foreach (json_decode(cache('daily_usages')) as $dailySales)
