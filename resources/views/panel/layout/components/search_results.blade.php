...
<ul class="m-0 p-0 list-none font-medium">
    ...
    <li class="p-2 px-3 border-solid border-b border-t-0 border-r-0 border-l-0 border-[--tblr-border-color] last:border-b-0 transition-colors hover:bg-slate-50  dark:hover:!bg-[rgba(255,255,255,0.05)]">
        @if($item->type == 'text')
            <a href="{{LaravelLocalization::localizeUrl(route('dashboard.user.openai.generator.workbook', $item->slug))}}" class="d-flex align-items-center text-heading !no-underline">
        ...
        @elseif($item->type == 'image')
            <a href="{{LaravelLocalization::localizeUrl(route('dashboard.user.openai.generator', $item->slug))}}" class="d-flex align-items-center text-heading !no-underline">
				<span class="avatar w-[43px] h-[43px] [&_svg]:w-[20px] [&_svg]:h-[20px] relative !me-2" style="background: {{$item->color}}">
					@if ( $item->image !== 'none' )
					{!! html_entity_decode($item->image) !!}
					@endif
...
@if(count($workbook_search)>0)
    ...
    <ul class="m-0 p-0 list-none">
        ...
        <li class="p-2 px-3 border-solid border-b border-t-0 border-r-0 border-l-0 border-[--tblr-border-color] last:border-b-0 transition-colors hover:bg-slate-50  dark:hover:!bg-[rgba(255,255,255,0.05)]">
			<a href="{{LaravelLocalization::localizeUrl(route('dashboard.user.openai.documents.single', $item->slug))}}" class="d-flex align-items-center text-heading !no-underline">
				<span class="avatar w-[43px] h-[43px] [&_svg]:w-[20px] [&_svg]:h-[20px] relative !me-2" style="background: {{$item->color}}">
					@if ( $item->image !== 'none' )
					{!! html_entity_decode($item->image) !!}
					@endif
