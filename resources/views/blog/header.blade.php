<header @class([
    ...
    <nav id="frontend-local-navbar" @class([
        ...
        <a class="site-logo relative basis-1/3 max-lg:basis-1/3" href="{{ route('index') }}">
            @if (isset($setting->logo_sticky))
                <img class="peer absolute start-0 top-1/2 -translate-y-1/2 translate-x-3 opacity-0 transition-all group-[.lqd-is-sticky]/header:translate-x-0 group-[.lqd-is-sticky]/header:opacity-100"
                    src="/{{ $setting->logo_sticky_path }}"
                    @if (isset($setting->logo_sticky_2x_path)) srcset="/{{ $setting->logo_sticky_2x_path }} 2x" @endif
                    alt="{{ $setting->site_name }} logo">
            @endif
            <img class="transition-all group-[.lqd-is-sticky]/header:peer-first:translate-x-2 group-[.lqd-is-sticky]/header:peer-first:opacity-0"
                src="/{{ $app_is_demo ? $setting->logo_sticky_path : $setting->logo_path }}"
        ...
        <div
            ...
            <ul
                class="flex items-center justify-center gap-14 whitespace-nowrap text-center max-xl:gap-10 max-lg:flex-col max-lg:items-start max-lg:gap-5 max-lg:p-10">
                @php
                ...
                <div class="group relative -mt-3 block border-t border-white/5 px-10 pb-5 pt-6 md:hidden">
                    ...
                    @foreach (LaravelLocalization::getSupportedLocales() as $localeCode => $properties)
                        @if (in_array($localeCode, explode(',', $settings_two->languages)))
                            <a class="block border-b border-black border-opacity-5 py-3 transition-colors last:border-none hover:bg-black hover:bg-opacity-5"
        ...
        <div class="flex basis-1/3 justify-end gap-2 max-lg:basis-2/3">
            @if (count(explode(',', $settings_two->languages)) > 1)
                <div class="group relative hidden md:block">
                    ...
                    <div
                        ...
                        @foreach (LaravelLocalization::getSupportedLocales() as $localeCode => $properties)
                            @if (in_array($localeCode, explode(',', $settings_two->languages)))
                                <a class="block border-b border-black border-opacity-5 px-3 py-3 transition-colors last:border-none hover:bg-black hover:bg-opacity-5"
                                    href="{{ LaravelLocalization::getLocalizedURL($localeCode, null, [], true) }}"
                                    rel="alternate"
                                    hreflang="{{ $localeCode }}">{{ country2flag(substr($properties['regional'], strrpos($properties['regional'], '_') + 1)) }}
                                    {{ $properties['native'] }}</a>
            ...
            @else
                <x-button link="{{ LaravelLocalization::localizeUrl(route('login')) }}" label="{!! __($fSetting->sign_in) !!}"
                    type="outline"
                    ...
                    text="{{ isset($page) && $page->titlebar_status ? 'text-white' : 'text-black' }} group-[.lqd-is-sticky]/header:text-black group-[.lqd-is-sticky]/header:hover:text-white" />
                <x-button link="{{ LaravelLocalization::localizeUrl(route('register')) }}" label="{!! __($fSetting->join_hub) !!}"
                    border="border-[2px] {{ isset($page) && $page->titlebar_status ? 'border-white' : 'border-black' }} !border-opacity-0"
                    bg="{{ isset($page) && $page->titlebar_status ? 'bg-white' : 'bg-black' }} !bg-opacity-10 hover:!bg-opacity-100 group-[.lqd-is-sticky]/header:bg-black group-[.lqd-is-sticky]/header:hover:!bg-opacity-100"
                    text="{{ isset($page) && $page->titlebar_status ? 'text-white' : 'text-black' }} group-[.lqd-is-sticky]/header:text-black group-[.lqd-is-sticky]/header:hover:text-white" />
            @endauth
...
@if ($app_is_demo)
    <div class="fixed bottom-8 start-8 z-50">
        <a class="inline-flex h-12 w-12 items-center justify-center rounded-full bg-[#262626] transition-all duration-300 hover:-translate-y-1 hover:scale-110 hover:shadow-md"
            href="https://codecanyon.net/item/magicai-openai-content-text-image-chat-code-generator-as-saas/45408109"
            target="_blank" title="{{ __('Buy on Envato') }}">
